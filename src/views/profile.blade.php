...
@section('buttons')
    ...
    @if (auth()->user()->roles->first()->name == 'espositore')
        <a onclick="saveData(this);" href="javascript:void(0);" class="btn btn-primary" data-toggle="tooltip"
            data-placement="bottom" title="{{ trans('generals.save') }}"><i class="fas fa-save"></i></a>
    @else
        <a onclick="updatePsw(this);" href="javascript:void(0);" class="btn btn-primary" data-toggle="tooltip"
            data-placement="bottom" title="{{ trans('generals.save') }}"><i class="fas fa-save"></i></a>
    @endif
@endsection
@section('content')
